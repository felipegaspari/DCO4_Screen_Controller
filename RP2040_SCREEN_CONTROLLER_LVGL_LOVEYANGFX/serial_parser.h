// Generic serial frame parser core for the screen controller.
// This is a local copy of the shared parser used on the mainboard/input/DCO,
// with a larger SERIAL_MAX_PAYLOAD to accommodate the screen's longer preset
// name frames ('q' with up to 16 characters + metadata).
//
// The parser itself is transport-agnostic: it only knows about command bytes,
// fixed payload lengths, and per-command callbacks.
...
// For the screen controller we need to support:
//   - mainboard<->screen: 'q' with [presetNumber + 16 chars]  => 17 bytes
//   - input<->screen:     'q' with [presetNumber + 16 chars + finish] => 18
// So we choose 18 as the maximum payload size.
static const uint8_t SERIAL_MAX_PAYLOAD = 18;
